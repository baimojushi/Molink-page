// services/mmwBanana.js —— MMW / Nano Banana 生图适配器
// 保持原 Snaptoshine 业务接口不变：submitImageRequest / checkExecution / downloadFile
// 特性：异步本地队列、任务持久化、模型 fallback、兼容多种返回图片格式。
const fs = require('fs');
...
const JOB_FILE = path.join(JOB_DIR, 'mmw-jobs.json');

const SERVER_BASE_URL = String(process.env.SERVER_BASE_URL || 'https://www.molink.art').replace(/\/+$/, '');
const API_BASE_URL = String(process.env.MMW_API_BASE_URL || 'https://api.mmw.ink').replace(/\/+$/, '');
const API_KEY = process.env.MMW_API_KEY || '';
const PRIMARY_MODEL = process.env.MMW_MODEL || 'gemini-3.1-flash-image';
const FALLBACK_MODELS = parseModelList(process.env.MMW_FALLBACK_MODELS || 'gemini-2.5-flash-image');
const ASPECT_RATIO = process.env.MMW_ASPECT_RATIO || '1:1';
const IMAGE_SIZE = process.env.MMW_IMAGE_SIZE || '1K';
const EXECUTION_COUNT = Math.max(1, parseInt(process.env.MMW_EXECUTION_COUNT || '5', 10));
const CONCURRENCY = Math.max(1, parseInt(process.env.MMW_CONCURRENCY || '2', 10));
const MAX_ATTEMPTS = Math.max(1, parseInt(process.env.MMW_MAX_ATTEMPTS || '3', 10));
const TIMEOUT_MS = Math.max(30000, parseInt(process.env.MMW_TIMEOUT_MS || '180000', 10));
const RETRY_BASE_DELAY_MS = Math.max(500, parseInt(process.env.MMW_RETRY_BASE_DELAY_MS || '3000', 10));
const REQUEST_MODE = String(process.env.MMW_REQUEST_MODE || 'openai').toLowerCase(); // openai | gemini
const INCLUDE_IMAGE_SIZE = process.env.MMW_INCLUDE_IMAGE_SIZE !== '0';
const INCLUDE_ASPECT_RATIO = process.env.MMW_INCLUDE_ASPECT_RATIO !== '0';
const INCLUDE_MODALITIES = process.env.MMW_INCLUDE_MODALITIES === '1';
const FORCE_IMAGE_MIME = process.env.MMW_FORCE_IMAGE_MIME || 'image/jpeg';
const EXTRA_BODY = safeJsonParse(process.env.MMW_EXTRA_BODY_JSON, {});
...
function selectModelForAttempt(job, attempt) {
  ...
  // 第 1 次用主模型；第 2 次开始优先使用备用模型，避免反复撞同一个上游 500。
...
function loadJobs() {
  try {
    ...
    console.warn(`⚠️ MMW 任务文件读取失败，将从空状态启动: ${e.message}`);
    return {};
  }
}
...
function saveJobs() {
  try {
    ...
    console.warn(`⚠️ MMW 任务文件写入失败: ${e.message}`);
  }
}
...
function recoverExistingJobs() {
  ...
  if (recoverable.length > 0) {
    console.log(`♻️ 恢复 MMW 生图任务 ${recoverable.length} 个`);
  }
...
function makeExecutionId() {
  return `mmw_${Date.now().toString(36)}_${crypto.randomBytes(8).toString('hex')}`;
}

function looksLikeMmwId(id) {
  return /^mmw_[a-z0-9]+_[a-f0-9]{12,}$/i.test(String(id || ''));
}
...
async function buildOpenAiMessages(userMessage) {
  const content = [];
  for (const m of normalizeUserMessage(userMessage)) {
    if (m.file_url || m.image_url || m.url) {
      ...
      content.push({ type: 'text', text: String(m.text) });
    }
  }
  if (content.length === 0) throw new Error('MMW 生图消息为空');
...
async function buildGeminiContents(userMessage) {
  ...
  if (parts.length === 0) throw new Error('MMW 生图消息为空');
...
async function callMmwOpenAi({ model, userMessage }) {
  if (!API_KEY) throw new Error('缺少 MMW_API_KEY');
  const messages = await buildOpenAiMessages(userMessage);
  const body = {
    model,
    messages,
    ...EXTRA_BODY
  };
  if (INCLUDE_ASPECT_RATIO) body.aspect_ratio = ASPECT_RATIO;
  if (INCLUDE_IMAGE_SIZE) body.image_size = IMAGE_SIZE;
  if (INCLUDE_MODALITIES) body.modalities = ['image'];
  ...
  if (res.status < 200 || res.status >= 300) {
    const err = new Error(`MMW API 返回错误 HTTP ${res.status}: ${res.text.substring(0, 1200)}`);
...
async function callMmwGemini({ model, userMessage }) {
  if (!API_KEY) throw new Error('缺少 MMW_API_KEY');
  const contents = await buildGeminiContents(userMessage);
  const body = {
    contents,
    generationConfig: {
  ...
  if (INCLUDE_ASPECT_RATIO) body.generationConfig.aspectRatio = ASPECT_RATIO;
  if (INCLUDE_IMAGE_SIZE) body.generationConfig.imageSize = IMAGE_SIZE;

  const encodedModel = encodeURIComponent(model);
  const url = `${API_BASE_URL}/v1beta/models/${encodedModel}:generateContent?key=${encodeURIComponent(API_KEY)}`;
  const res = await httpRequestBuffer(url, {
    method: 'POST',
  ...
  const data = parseApiResponse(res.text);
  if (res.status < 200 || res.status >= 300) {
    const err = new Error(`MMW Gemini 接口返回错误 HTTP ${res.status}: ${res.text.substring(0, 1200)}`);
    err.status = res.status;
    err.data = data;
    throw err;
  }
...
async function persistImageCandidate(candidate, executionId) {
  ...
  if (!buffer || buffer.length < 80) throw new Error('MMW 返回图片为空或过小');
  ...
  try {
    ...
    console.warn(`⚠️ MMW 结果图转 PNG 失败，保存原始格式: ${e.message}`);
  }
...
async function generateJob(job) {
  ...
  while (job.attempts < job.maxAttempts) {
    ...
    console.log(`🎨 MMW 生图开始 id=${job.id} attempt=${attempt}/${job.maxAttempts} model=${model}${model !== job.model ? ' fallback=1' : ''}`);
    try {
      const response = REQUEST_MODE === 'gemini'
        ? await callMmwGemini({ model, userMessage: job.userMessage })
        : await callMmwOpenAi({ model, userMessage: job.userMessage });
      const candidates = extractImageCandidates(response);
      if (!candidates.length) {
        throw new Error(`MMW API 已返回但未解析到图片: ${JSON.stringify(response).substring(0, 1200)}`);
      }
      ...
      saveJobs();
      console.log(`✅ MMW 生图完成 id=${job.id} model=${model} url=${job.imageUrl}`);
      return;
    } catch (e) {
      ...
      const modelHint = modelCandidates.length > 1 && attempt < modelCandidates.length
        ...
        : '';
      console.warn(`⚠️ MMW 生图失败 id=${job.id} attempt=${attempt}: ${e.message}${canRetry ? modelHint + '，准备重试' : ''}`);
      if (!canRetry) break;
...
function processQueue() {
  while (running < CONCURRENCY && queue.length > 0) {
    ...
    generateJob(job)
      .catch(e => {
        console.error(`MMW 队列任务异常 id=${id}:`, e.message);
        if (jobs[id]) {
...
async function submitImageRequest({ userMessage, executionCount }) {
  ...
  saveJobs();
  console.log(`📤 已提交 MMW 生图队列 数量=${ids.length} model=${PRIMARY_MODEL} fallbacks=${FALLBACK_MODELS.join('|') || '-'} ids=${ids.join(',')}`);
  return ids;
}
...
async function checkExecution(executionId) {
  ...
  if (!job) {
    if (looksLikeMmwId(executionId)) return { status: 'failed', raw: { error: 'MMW job not found', executionId } };
